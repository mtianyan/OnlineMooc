...
import {addCourseComments, queryCourseComments, removeCourseComments, updateCourseComments} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryCourse} from '@/pages/CourseList/service';import {queryUserProfile} from '@/pages/UserProfileList/service';
...
const TableList = () => {
  ...
  try {
    await addCourseComments({...fields});
    hide();
  ...
  try {
    await updateCourseComments(value, current_id);
    hide();
  ...
  try {
    ...
    await removeCourseComments(ids);
    hide();
  ...
  const base_columns = [{
        ...
        const children = courseForeignKeyList.map((item) => {
  ...
  useEffect(() => {
    queryCourse().then(value => {
      setCourseForeignKeyList(value.data);
    });
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索课程评论 " onSearch={value => {
            setParamState({
              search: value,
            });
        ...
        request={(params, sorter, filter) => queryCourseComments({...params, sorter, filter})}
