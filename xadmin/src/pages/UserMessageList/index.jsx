...
import {addUserMessage, queryUserMessage, removeUserMessage, updateUserMessage} from './service';
...
const TableList = () => {
  ...
  try {
    await updateUserMessage(value, current_id);
    hide();
  ...
  const base_columns = [{
      ...
      const is_value = form.getFieldValue('has_read');
      if (type === "form" && !is_value) {
        form.setFieldsValue({'has_read': false});
      }
      return <Switch checked={is_value} onClick={(checked) => {
        form.setFieldsValue({'has_read': checked});
      }} />;
    },
                            },{
          title: '添加时间',
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索用户消息 " onSearch={value => {
            setParamState({
              search: value,
            });
        ...
        request={(params, sorter, filter) => queryUserMessage({...params, sorter, filter})}
