...
import {addRead, queryRead, removeRead, updateRead} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryTeacher} from '@/pages/AutoGenPage//TeacherList/service';
...
const TableList = () => {
  ...
  const base_columns = [{
          ...
          return BooleanDisplay(text);
                                      ...
                                      renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
          const is_value = form.getFieldValue('isRecommend');
          if (type === "form" && !is_value) {
            form.setFieldsValue({'isRecommend': false});
          }
          return <Switch checked={is_value} onClick={(checked) => {
            form.setFieldsValue({'isRecommend': checked});
          }} />;
        },
                                },{
                              title: '作者',
            ...
            const children = authorForeignKeyList.map((item) => {
  ...
  const [authorForeignKeyList, setAuthorForeignKeyList] = useState([]);
      useEffect(() => {
        queryTeacher().then(value => {
          setAuthorForeignKeyList(value.data);
        });
      }, []);
