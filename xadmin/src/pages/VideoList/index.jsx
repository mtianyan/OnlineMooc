...
import {addVideo, queryVideo, removeVideo, updateVideo} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryLesson} from '@/pages/LessonList/service';
...
const TableList = () => {
  ...
  try {
    await updateVideo(value, current_id);
    hide();
  ...
  const base_columns = [{
        ...
        const children = lessonForeignKeyList.map((item) => {
  ...
  useEffect(() => {
    queryLesson().then(value => {
      setLessonForeignKeyList(value.data);
    });
  }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索视频 " onSearch={value => {
            setParamState({
              search: value,
            });
        ...
        request={(params, sorter, filter) => queryVideo({...params, sorter, filter})}
